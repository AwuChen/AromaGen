...
// 50-word table is provided -- expertRatioFor_() degrades gracefully
// (returns null) for any word not yet in this object, rather than erroring,
...
/** The expert-derived ratio string for `word`, or null if not yet set in
 * EXPERT_RATIOS. */
function expertRatioFor_(word) {
  return Object.prototype.hasOwnProperty.call(EXPERT_RATIOS, word) ? EXPERT_RATIOS[word] : null;
}
...
var SHEET_SCHEMAS_ = {
  // "condition" appended at the end -- safely migratable (ensureSheetHeaders_
  // handles trailing-column additions automatically); existing rows just show
  // blank for it, matching whatever their frozen plan_json.condition already is.
  participants: ["participant_name", "seq_index", "odorant_set", "feedback_type",
    "status", "created_at", "completed_at", "condition"],
